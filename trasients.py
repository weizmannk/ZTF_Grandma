            ...
            if save_file_name[i] in f:   
                ...
                try:
                    ...
                    if filter == "g":
                        ...
                        axs.errorbar(t_dec-tmin_inc, y_dec+offset, dy_dec, fmt=g_fmt, c=g_color, label=filter_rename+ " "+save_file_name[i])
                        axs.plot(t_dec - tmin_inc, max_likelihood_dec+offset, c=col_dec, ls=ls_dec,  linewidth=1)

                        if save_file_name[i] == "ZTF21abbzjeq":                          
                            indy_dec = np.where(y_dec == np.max(y_dec))
                            x_dec = t_dec[indy_dec]-15
                            y_dec = np.max(y_dec)+0.2 
                            axs.text(x_dec, y_dec, r'   %.3f ' % slope_dec, rotation=-40, color="navy", fontsize=10)
                            ...
                            y_dec = np.max(y_dec) +0.35
                            axs.text(x_dec, y_dec, r'  %.3f ' % slope_dec, rotation=-45, color="navy", fontsize=10)

                        elif save_file_name[i] == "ZTF21abfaohe":  
                            indy_dec = np.where(y_dec == np.max(y_dec))
                            x_dec = t_dec[indy_dec]-6.5
                            y_dec = np.max(y_dec)+0.5
                            axs.text(x_dec, y_dec, r'  %.3f ' % slope_dec,rotation=-30, color="navy", fontsize=10)
                        
                        elif save_file_name[i] == "ZTF21acceboj":
                            indy_dec = np.where(y_dec == np.max(y_dec))
                            x_dec = t_dec[indy_dec]-10
                            y_dec = np.max(y_dec)+0.5
                            axs.text(x_dec, y_dec, r'  %.3f ' % slope_dec,  rotation=0, color="navy", fontsize=10)
                        ...
                        if save_file_name[i] == "ZTF21acceboj":
                            ...
                            axs.errorbar(t_dec-tmin_inc, y_dec , dy_dec, fmt=r_fmt, c=r_color, label=filter_rename)
                            axs.plot(t_dec - tmin_inc, max_likelihood_dec, c=col_dec, ls=ls_dec,  linewidth=1)
                            
                            if save_file_name[i] == "ZTF21abbzjeq":                          
                                indy_dec = np.where(y_dec == np.max(y_dec))
                                x_dec = t_dec[indy_dec]-23
                                y_dec = np.max(y_dec)-0.6 
                                axs.text(x_dec, y_dec, r'   %.3f ' % slope_dec, rotation=-20, color="navy", fontsize=10)
                            
                            elif save_file_name[i] == "ZTF21absvlrr":
                                indy_dec = np.where(y_dec == np.max(y_dec))
                                x_dec = t_dec[indy_dec]-10
                                y_dec = np.max(y_dec) -0.15
                                axs.text(x_dec, y_dec, r'   %.3f ' % slope_dec, rotation=-15, color="navy", fontsize=10)
                ...
                try:
                    ...
                    if filter == "g":
                        ...
                        axs.errorbar(t_dec-tmin_dec, y_dec +offset, dy_dec, fmt=g_fmt, c=g_color, label= filter+ " "+save_file_name[i])
                        axs.plot(t_dec - tmin_dec, max_likelihood_dec + offset, 'k:', linewidth=1)
                        indy_dec = np.where(y_dec == np.max(y_dec))
                        x_dec = t_dec[indy_dec]-4
                        y_dec = np.max(y_dec)+0.7 #  #np.max(max_likelihood_dec)
                        axs.text(x_dec, y_dec, r'  %.3f  ' % slope_dec,  rotation=-50,  color="navy", fontsize=10)
                            
            
                    else :
                        axs.errorbar(t_dec-tmin_dec, y_dec, dy_dec, fmt=r_fmt, c=r_color, label=filter)
                        axs.plot(t_dec - tmin_dec, max_likelihood_dec, 'k:', linewidth=1)

                        indy_dec = np.where(y_dec == np.max(y_dec))
                        x_dec = t_dec[indy_dec]-6
                        y_dec = np.max(y_dec)-0.3  # np.max(max_likelihood_dec)
        
                        axs.text(x_dec, y_dec, r'  %.3f  ' % slope_dec,rotation=-55,  color="navy", fontsize=10)
                ...
                try:
                    ...
                    axs.errorbar(t_dec-tmin_dec, y_dec, dy_dec, fmt=r_fmt, c=r_color, label=filter+ " "+save_file_name[i] )
                    axs.plot(t_dec - tmin_dec, max_likelihood_dec, 'g:', linewidth=1)

                    indy_dec = np.where(y_dec == np.max(y_dec))
                    x_dec = t_dec[indy_dec]+1.5
                    y_dec = np.max(y_dec) - 0.03  # np.max(max_likelihood_dec)
...
# ======================================================================
df_f.to_csv(saveDir+"/"+"slopeF.csv")
                

#axs.margins(y=0.7)
axs.text(45, 17, "0.6 mag offset on the g-band", rotation=-45,  color="k", fontsize=10)

axs.set_ylabel(r'Magnitude')
axs.set_xlabel(r'First time since the peak mag detection')
